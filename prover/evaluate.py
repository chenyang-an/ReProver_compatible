...
def _get_theorems_from_files(
    ...
    data = json.load(open(os.path.join(data_path, f"{split}.json")))[0:2]
    theorems = []
    positions = []

    print("_get_theorems_from_files is called, the data path is ", os.path.join(data_path, f"{split}.json"))
    print(data[0])
...
def main() -> None:
    ...
    logger.info(args)
    print("Now we start to get the theorem.")

    repo, theorems, positions = _get_theorems(args)

    print("We have gotten the theorem, now we start to initialize the prover.")
    ...
    print("Prover is initialized, now we start to search.")
